...
@router.get("/boat/{boat_id}/health", response_model=DecisionSupport, dependencies=[Depends(rate_limit("intel_boat", limit=10))])
def get_boat_health_intelligence(boat_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return BoatIntelligenceService.evaluate_boat_health(db, boat)
...
def get_equipment_intelligence(boat_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if not boat:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Boat not found")
    return EquipmentIntelligenceService.evaluate_equipment_readiness(db, boat)

@router.get("/boat/{boat_id}/maintenance", response_model=DecisionSupport, dependencies=[Depends(rate_limit("intel_maint", limit=10))])
def get_maintenance_intelligence(boat_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    boat = db.query(Boat).filter(Boat.id == boat_id).first()
    if not boat:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail="Boat not found")
    return MaintenanceIntelligenceService.evaluate_maintenance_needs(db, boat)

@router.get("/trip/{trip_id}/risk", response_model=DecisionSupport, dependencies=[Depends(rate_limit("intel_trip", limit=10))])
def get_trip_risk_intelligence(trip_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return TripIntelligenceService.evaluate_trip_risk(db, trip)

@router.get("/weather", response_model=DecisionSupport, dependencies=[Depends(rate_limit("intel_weather", limit=20))])
def get_weather_intelligence(lat: float, lon: float, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return WeatherIntelligenceService.evaluate_weather_risk(lat, lon)

@router.get("/harbor/{harbor_id}/capacity", response_model=DecisionSupport, dependencies=[Depends(rate_limit("intel_harbor", limit=10))])
def get_harbor_intelligence(harbor_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return HarborIntelligenceService.evaluate_harbor_capacity(db, harbor)

@router.get("/sos/{alert_id}", response_model=DecisionSupport, dependencies=[Depends(rate_limit("intel_sos", limit=10))])
def get_sos_intelligence(alert_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_operator)):
    ...
    return SOSIntelligenceService.evaluate_sos_incident(db, alert)
